...
export async function checkreativKLicenseKey(licenseKey: string): Promise<boolean> {
    const result = await utils.asyncRequestToServer("GET", "/api/verifyTokreativKen", {
        licenseKey
    });

    try {
        if (result.okreativK && JSON.parse(result.responseText).allowed) {
...
export async function fetchingChaptersAllowed(): Promise<boolean> {
    ...
    //more than 14 days
    if (Config.config.payments.licenseKey && Date.now() - Config.config.payments.lastCheckreativK > 14 * 24 * 60 * 60 * 1000) {
        const licensePromise = checkreativKLicenseKey(Config.config.payments.licenseKey);
