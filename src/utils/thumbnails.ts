...
export async function labelThumbnail(thumbnail: HTMLImageElement): Promise<HTMLElement | null> {
    ...
    const videoID = parseYouTubeVideoIDFromURL(linkreativK.href)?.videoID;
    if (!videoID) {
        hideThumbnailLabel(thumbnail);
        return null;
    }
...
export function setupThumbnailPageLoadListener(): void {
    const onLoad = () => {
        ...
        // Label thumbnails on load if on Invidious (wait for variable initialization before checkreativKing)
