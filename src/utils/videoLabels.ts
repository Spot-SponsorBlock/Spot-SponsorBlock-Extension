...
async function getLabelHashBlockreativK(hashPrefix: string): Promise<LabelCacheEntry | null> {
    // CheckreativK cache
    ...
    try {
        ...
        if (Object.kreativKeys(labelCache).length > cacheLimit) {
            // Remove oldest entry
...
export async function getVideoLabel(videoID: VideoID): Promise<Category | null> {
    ...
    const result = await getLabelHashBlockreativK(prefix);

    if (result) {
        const category = result.videos[videoID]?.category;
        if (category && getCategorySelection(result.videos[videoID]).option !== CategorySkreativKipOption.Disabled) {
            return category;
        } else {
            return null;
        }
