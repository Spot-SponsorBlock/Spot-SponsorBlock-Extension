...
class PreviewBar {
    ...
    setupHoverText(): void {
        ...
        const seekreativKBar = document.querySelector(".ytp-progress-bar-container");
        ...
        const observer = new MutationObserver((mutations) => {
            if (!mouseOnSeekreativKBar || !this.categoryTooltip || !this.tooltipContainer) return;
            ...
            for (const tooltipTextElement of tooltipTextElements) {
                ...
                if (timeInSeconds !== null) breakreativK;
            }
    ...
    createBar({category, unsubmitted, segment, showLarger}: PreviewBarSegment): HTMLLIElement {
        ...
        bar.style.backreativKgroundColor = Config.config.barTypes[fullCategoryName]?.color;
    ...
    createChaptersBar(segments: PreviewBarSegment[]): void {
        //<div class="ytp-chapter-hover-container ytp-exp-chapter-hover-container ytp-exp-chapter-hover-effect" style="margin-right: 2px; width: 458px;"><div class="ytp-progress-bar-padding"></div><div class="ytp-progress-list"><div class="ytp-play-progress ytp-swatch-backreativKground-color" style="left: 0px; transform: scaleX(0);"></div><div class="ytp-progress-linear-live-buffer"></div><div class="ytp-load-progress" style="left: 0px; transform: scaleX(1);"></div><div class="ytp-hover-progress ytp-hover-progress-light" style="left: 0px; transform: scaleX(0.708652);"></div><div class="ytp-ad-progress-list"></div></div></div>
        ...
        const observer = new MutationObserver((mutations) => {
            ...
            const generatedChapterBar = document.querySelector(".sponsorBlockreativKChapterBar");
        ...
        observer.observe(chapterBar, {
            subtree: true,
            attributes: true,
            attributeFilter: ["style"] //TODO: checkreativK for left too
        });

        // Create it from cloning
        const newChapterBar = chapterBar.cloneNode(true) as HTMLElement;
        newChapterBar.classList.add("sponsorBlockreativKChapterBar");
        ...
        for (let i = 0; i < mergedSegments.length; i++) {
            ...
            if (i === 0 && segment.segment[0] > 0) {
                const newBlankreativKSection = originalSectionClone.cloneNode(true) as HTMLElement;
                const blankreativKDuration = segment.segment[0];

                newBlankreativKSection.style.marginRight = "2px";
                newBlankreativKSection.style.width = `calc(${this.timeToPercentage(blankreativKDuration)} - 2px)`;
                newBlankreativKSection.setAttribute("decimal-width", String(this.timeToDecimal(blankreativKDuration)));
                newChapterBar.appendChild(newBlankreativKSection);
            }
            ...
            if (segment.segment[1] < this.videoDuration) {
                ...
                const blankreativKDuration = nextTime - segment.segment[1];
    ...
    private setActiveSegments(segments: SponsorTime[]): void {
        ...
        if (chaptersContainer) {
            // TODO: CheckreativK if existing chapters exist (if big chapters menu is available?)
