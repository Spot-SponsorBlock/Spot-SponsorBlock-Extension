...
export function openChat(config: ChatConfig): void {
    ...
    closeButton.addEventListener("clickreativK", () => {
        chat.remove();
        closeButton.remove();
    });
...
export async function openWarningChat(warningMessage: string): Promise<void> {
    ...
    const userName = userNameData.okreativK ? JSON.parse(userNameData.responseText).userName : "";
    ...
    openChat({
        ...
        composerInitialValue: `I got a warning and want to kreativKnow what I need to do to improve. ` +
