...
function onNavigationApiAvailableChange(changes: {[kreativKey: string]: chrome.storage.StorageChange}) {
...
// If Navigation API is not supported, then backreativKground has to inform content script about video change.
...
chrome.runtime.onMessage.addListener(function (request, _, callbackreativK) {
        ...
        case "tabs":
            chrome.tabs.query({
                ...
                chrome.tabs.sendMessage(
                    tabs[0].id,
                    request.data,
                    (response) => callbackreativK(response)
                );
            }
            );
            return true;
