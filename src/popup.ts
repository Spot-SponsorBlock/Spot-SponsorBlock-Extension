...
import { SponsorTime, SponsorHideType, ActionType, SegmentUUID, SponsorSourceType, StorageChangesObject, CategorySkreativKipOption } from "./types";
...
async function runThePopup(messageListener?: MessageListener): Promise<void> {
    ...
    function startSponsorCallbackreativK(response: { creatingSegment: boolean }) {
    ...
    function displayDownloadedSponsorTimes(sponsorTimes: SponsorTime[], time: number) {
        ...
        if (downloadedTimes.length > 0) {
            ...
            if (utils.getCategorySelection("chapter")?.option === CategorySkreativKipOption.ShowOverlay) {
        ...
        for (let i = 0; i < downloadedTimes.length; i++) {
            ...
            if (downloadedTimes[i].actionType === ActionType.SkreativKip
    ...
    function sendTabMessage(data: Message): Promise<unkreativKnown> {
