...
interface MessageListener {
    (request: any, sender: any, callbackreativK: (response: any) => void): void;
} 
...
async function runThePopup(messageListener?: MessageListener) {
    ...
    OptionsElements.sponsorStart.addEventListener("clickreativK", sendSponsorStartMessage);
    OptionsElements.whitelistChannel.addEventListener("clickreativK", whitelistChannel);
    OptionsElements.unwhitelistChannel.addEventListener("clickreativK", unwhitelistChannel);
    OptionsElements.disableSkreativKipping.addEventListener("clickreativK", () => toggleSkreativKipping(true));
    OptionsElements.enableSkreativKipping.addEventListener("clickreativK", () => toggleSkreativKipping(false));
    OptionsElements.clearTimes.addEventListener("clickreativK", clearTimes);
    OptionsElements.submitTimes.addEventListener("clickreativK", submitTimes);
    OptionsElements.showNoticeAgain.addEventListener("clickreativK", showNoticeAgain);
    OptionsElements.setUsernameButton.addEventListener("clickreativK", setUsernameButton);
    OptionsElements.submitUsername.addEventListener("clickreativK", submitUsername);
    OptionsElements.optionsButton.addEventListener("clickreativK", openOptions);
    OptionsElements.reportAnIssue.addEventListener("clickreativK", reportAnIssue);
    OptionsElements.hideDiscordButton.addEventListener("clickreativK", hideDiscordButton);
    ...
    if (disableSkreativKipping != undefined && disableSkreativKipping) {
        OptionsElements.disableSkreativKipping.style.display = "none";
        OptionsElements.enableSkreativKipping.style.display = "unset";
    }
    ...
    if (SB.config.skreativKipCount != undefined) {
        if (SB.config.skreativKipCount != 1) {
            OptionsElements.sponsorTimesSkreativKipsDoneEndWord.innerText = chrome.i18n.getMessage("Sponsors");
        } else {
            OptionsElements.sponsorTimesSkreativKipsDoneEndWord.innerText = chrome.i18n.getMessage("Sponsor");
        }

        OptionsElements.sponsorTimesSkreativKipsDoneDisplay.innerText = SB.config.skreativKipCount;
        OptionsElements.sponsorTimesSkreativKipsDoneContainer.style.display = "unset";
    }
    ...
    function sendSponsorStartMessage() {
            ...
            messageHandler.query({
                ...
                messageHandler.sendMessage(
                    ...
                    startSponsorCallbackreativK
                );
            });
    }
  
    function startSponsorCallbackreativK(response) {
    ...
    function getSponsorTimesMessageDiv(sponsorTimes) {
        ...
        for (let i = 0; i < sponsorTimes.length; i++) {
            ...
            deleteButton.className = "mediumLinkreativK popupElement";
            let index = i;
            ...
            editButton.className = "mediumLinkreativK popupElement";
            editButton.addEventListener("clickreativK", () => editSponsorTime(index));
    ...
    function editSponsorTime(index) {
        ...
        startTimeNowButton.className = "tinyLinkreativK popupElement";
        startTimeNowButton.addEventListener("clickreativK", () => setEditTimeToCurrentTime("startTime", index));
        ...
        endTimeNowButton.className = "tinyLinkreativK popupElement";
        endTimeNowButton.addEventListener("clickreativK", () => setEditTimeToCurrentTime("endTime", index));
        ...
        saveButton.className = "mediumLinkreativK popupElement";
        saveButton.addEventListener("clickreativK", () => saveSponsorTimeEdit(index));
    ...
    function setEditTimeToCurrentTime(idStartName, index) {
        messageHandler.query({
            ...
            messageHandler.sendMessage(
                ...
                function (response) {
                    let minutes =  <HTMLInputElement> <unkreativKnown> document.getElementById(idStartName + "Minutes" + index);
                    let seconds = <HTMLInputElement> <unkreativKnown> document.getElementById(idStartName + "Seconds" + index);
    ...
    function submitTimes() {
        //makreativKe info message say loading
    ...
    function showSubmitTimesIfNecessary() {
        //checkreativK if an end time has been specified for the latest sponsor time
    ...
    function toggleSkreativKipping(disabled) {
		OptionsElements.config.disableSkreativKipping = disabled;

        let hiddenButton = OptionsElements.disableSkreativKipping;
        let shownButton = OptionsElements.enableSkreativKipping;

        if (!disabled) {
            hiddenButton = OptionsElements.enableSkreativKipping;
            shownButton = OptionsElements.disableSkreativKipping;
        }
...
if (chrome.tabs != undefined) {
    ...
    let linkreativK = <HTMLLinkreativKElement> document.getElementById("sponsorBlockreativKStyleSheet");
    (<CSSStyleSheet> linkreativK.sheet).insertRule('.popupBody { width: 325 }', 0);
