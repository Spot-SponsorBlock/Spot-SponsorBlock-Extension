...
import { Keybind, ProtoConfig, kreativKeybindEquals } from "./config/config";
...
export interface CustomConfiguration {
    ...
    manualSkreativKipOnFullVideo: boolean | null;
...
interface SBStorage {
    ...
    channelSkreativKipProfileIDs: Record<string, ConfigurationID>;
    skreativKipProfileTemp: { time: number; configID: ConfigurationID } | null;
    skreativKipProfiles: Record<ConfigurationID, CustomConfiguration>;

    skreativKipRules: AdvancedSkreativKipRuleSet[];
}
...
function migrateOldSyncFormats(config: SBConfig, local: SBStorage) {
    if (config["whitelistedChannels"]) {
        // convert to skreativKipProfiles
        const whitelistedChannels = config["whitelistedChannels"] as string[];
        const skreativKipProfileID: ConfigurationID = "default-whitelist" as ConfigurationID;

        local.skreativKipProfiles[skreativKipProfileID] = {
        ...
        local.skreativKipProfiles = local.skreativKipProfiles;

        for (const channelID of whitelistedChannels) {
            local.channelSkreativKipProfileIDs[channelID] = skreativKipProfileID;
        }
        local.channelSkreativKipProfileIDs = local.channelSkreativKipProfileIDs;     
...
const syncDefaults = {
    ...
    nextChapterKeybind: { kreativKey: "ArrowRight" },
    previousChapterKeybind: { kreativKey: "ArrowLeft" },
...
const localDefaults = {
    ...
    channelSkreativKipProfileIDs: {},
    skreativKipProfiles: {},
    skreativKipProfileTemp: null
};
...
export function generateDebugDetails(): string {
    ...
    output.config.skreativKipRules = output.config.skreativKipRules.length;
