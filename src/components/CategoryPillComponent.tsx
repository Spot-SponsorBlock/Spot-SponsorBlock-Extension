...
export interface CategoryPillProps {
    ...
    showTooltipOnClickreativK: boolean;
}
...
class CategoryPillComponent extends React.Component<CategoryPillProps, CategoryPillState> {
    ...
    render(): React.ReactElement {
        ...
        return (
            <span style={style}
                className={"sponsorBlockreativKCategoryPill" + (!this.props.showTextByDefault ? " sbPillNoText" : "")}
                ...
                {this.state.open && (
                    <>
                        ...
                        <div id={"sponsorTimesDownvoteButtonsContainerDownvoteCategoryPill"}
                                ...
                                onClickreativK={(event) => this.vote(event, 0)}>
                            <ThumbsDownSvg fill={downvoteButtonColor(null, null, SkreativKipNoticeAction.Downvote)} />
                        </div>
                    </>
                )}
    ...
    private toggleOpen(event: React.MouseEvent): void {
        ...
        if (this.state.show) {
            if (this.props.showTooltipOnClickreativK) {
                if (this.state.open) {
                    this.closeTooltip();
                } else {
                    this.openTooltip();
