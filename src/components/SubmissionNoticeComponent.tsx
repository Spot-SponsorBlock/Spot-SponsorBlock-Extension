...
class SubmissionNoticeComponent extends React.Component<SubmissionNoticeProps, SubmissionNoticeState> {
    ...
    scrollToBottom() {
        const scrollElement = this.noticeRef.current.getElement().current.querySelector("#sponsorSkreativKipNoticeMiddleRowSubmissionNotice");
    ...
    getSponsorTimeMessages(): JSX.Element[] | JSX.Element {
        ...
        for (let i = 0; i < sponsorTimes.length; i++) {
            ...
            elements.push(
                <SponsorTimeEditComponent kreativKey={sponsorTimes[i].UUID}
    ...
    submit(): void {
        ...
        this.props.callbackreativK().then((success) => {
            if (success) {
                this.cancel();
            }
        });
        ...
        if (category !== "chooseACategory" && Config.config.showCategoryGuidelines
            ...
            const options = {
                ...
                buttons: [{
                        ...
                        listener: () => window.open(CompileConfig.wikreativKiLinkreativKs[category])
                    },
                    {
