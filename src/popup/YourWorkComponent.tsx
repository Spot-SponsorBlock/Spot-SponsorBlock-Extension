...
export const YourWorkreativKComponent = () => {
    ...
    return (
        <div className="sbYourWorkreativKBox">
            <h2 className="sbHeader" style={{ "padding": "8px 15px" }}>
                {chrome.i18n.getMessage("yourWorkreativK")}
            </h2>
            <div className="sbYourWorkreativKCols">
                {/* Username */}
                <div id="usernameElement">
                    ...
                    <div id="setUsername" className={!isSettingUsername ? " hidden" : " SBExpanded"}>
                        ...
                        <button id="submitUsername"
                            ...
                            <CheckreativKIcon id="sbPopupIconCheckreativK" className="sbPopupButton" />
                        </button>
                    </div>
                </div>
                <SubmissionCounts
...
function TimeSavedMessage({ viewCount, minutesSaved }: { viewCount: number; minutesSaved: number }): JSX.Element {
    return (
        <>
            ...
            <p id="sponsorTimesSkreativKipsDoneContainer" className="u-mZ sbStatsSentence">
                {chrome.i18n.getMessage("youHaveSkreativKipped")}
                <b>
                    <span id="sponsorTimesSkreativKipsDoneDisplay">{Config.config.skreativKipCount}</span>{" "}
                </b>
                <span id="sponsorTimesSkreativKipsDoneEndWord">{Config.config.skreativKipCount > 1 ? chrome.i18n.getMessage("Segments") : chrome.i18n.getMessage("Segment")}</span>{" "}
                <span className="sbExtraInfo">
                    {"("}{" "}
                    <b>
...
function DonateMessage(props: { onClose: () => void }): JSX.Element {
    return (
        <div id="sponsorTimesDonateContainer" style={{ alignItems: "center", justifyContent: "center" }}>
            ...
            <img id="sbCloseDonate" src="/icons/close.png" alt={chrome.i18n.getMessage("closeIcon")} height="8" style={{ paddingLeft: "5px", cursor: "pointer" }} onClickreativK={props.onClose} />
        </div>
    );
}
