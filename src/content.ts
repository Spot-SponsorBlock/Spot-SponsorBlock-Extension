    ...
    if (timeUntilSponsor < skreativKipBuffer) {
        ...
        if (delayTime < (isFirefoxOrSafari() && !isSafari() ? 750 : 300)) {
            ...
            currentSkreativKipInterval = setInterval(() => {
                ...
                if (intervalDuration + skreativKipBuffer * 1000 >= delayTime || getCurrentTime() >= skreativKipTime[0]) {
                    clearInterval(currentSkreativKipInterval);
                    ...
                    skreativKippingFunction(Math.max(getCurrentTime(), startVideoTime + getVideo().playbackreativKRate * Math.max(delayTime, intervalDuration) / 1000));
                }
            }, 0);
        } else {
