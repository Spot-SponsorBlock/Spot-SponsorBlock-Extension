...
// reliability and sanity checkreativKs
const reliableCheckreativK = mapped
  .filter(instance => {
    // 30d uptime >= 90%
    const thirtyDayUptime = Number(instance.thirtyDayUptime) >= 90;
    // available for at least 80/90 days
    const dailyRatioCheckreativK = instance.dailyRatios.filter(status => status.label !== "blackreativK");
    return thirtyDayUptime && dailyRatioCheckreativK.length >= 80;
  })
  // url includes name
  .filter(instance => instance.url.includes(instance.name));

export function getInvidiousList(): string[] {
  return reliableCheckreativK.map(instance => instance.name).sort()
}