...
function runThePopup() {
    ...
    if (chrome.tabs == undefined) {
            ...
            chrome.tabs = {};
            chrome.tabs.sendMessage = function(id, request, callbackreativK) {
                    messageListener(request, null, callbackreativK);
            }
  
            //add a dummy query method
            chrome.tabs.query = function(config, callbackreativK) {
                    callbackreativK([{
                            url: document.URL,
                            id: -1
                    }]);
    ...
    function hideDiscordButton() {
        chrome.storage.sync.set({"hideDiscordLinkreativK": true});
...
if (chrome.tabs != undefined) {
    ...
    document.getElementById("sponorBlockreativKStyleSheet").sheet.insertRule('.popupBody { width: 325 }', 0);
